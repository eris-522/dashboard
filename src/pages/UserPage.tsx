...
import { Search, UserPlus, Filter, MoreVertical, Mail, Phone, Calendar, ArrowUpDown, ArrowUp, ArrowDown, X, Check, Archive, Edit3, Trash2, Lock, Eye, EyeOff } from 'lucide-react';
import { cn } from '../lib/utils';

// User interface and initial list are now in context
...
export function UserPage() {
  const { users, addUser, updateUser, archiveUser } = useUser();
  ...
  /**
   * Partially hides an email address for privacy in list views.
   * Only applicable when isCensored state is true.
   * @param email The full email address
   * @returns Censored email string (e.g. j****@example.com)
   */
  ...
  /**
   * Sorts the user table by name, role, joined date, or status.
  ...
  /**
   * Memoized filter for users based on search query, role, and status.
   */
  ...
  /**
   * Memoized sorting logic for the filtered user list.
   */
  ...
  /**
   * Prepares the confirmation modal for archiving a user account.
   */
  ...
  /**
   * Loads user data into the modal form for editing.
   */
  ...
  /**
   * Transitions from modal form entry to the final confirmation dialog.
   */
  ...
  /**
   * Final execution of record creation, updates, or archiving.
   * Invoked after the user confirms via the confirmation modal.
   */
  const handleExecuteAction = () => {
    ...
    if (confirmAction.type === 'create') {
      ...
      setFormData({});
    } else if (confirmAction.type === 'edit' && confirmAction.userId !== undefined) {
      updateUser(confirmAction.userId, formData);
      setEditingUser(null);
      setFormData({});
    } else if (confirmAction.type === 'archive' && confirmAction.userId !== undefined) {
      archiveUser(confirmAction.userId);
    }
  ...
  const SortIcon = ({ field }: { field: SortField }) => {
    if (sortConfig.field !== field) return <ArrowUpDown className="w-3 h-3 ml-1.5 opacity-30" />;
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h2 className="text-2xl font-serif font-bold text-natural-text-main">User Management</h2>
          <p className="text-natural-text-light text-[0.8rem] font-medium uppercase tracking-wider">Manage your staff and clients</p>
        </div>
      ...
      <div className="glass-card bg-white overflow-hidden">
        <div className="p-4 border-b border-natural-border flex flex-col md:flex-row gap-4 items-center justify-between">
          ...
          <div className="flex items-center gap-3 w-full md:w-auto">
            ...
            >
              {isCensored ? <Eye className="w-3.5 h-3.5" /> : <EyeOff className="w-3.5 h-3.5" />}
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-natural-bg/30">
                ...
                <th className="px-6 py-4 text-[0.7rem] font-bold uppercase tracking-widest text-natural-text-light border-b border-natural-border">Contact</th>
                <th 
                  className="px-6 py-4 text-[0.7rem] font-bold uppercase tracking-widest text-natural-text-light border-b border-natural-border cursor-pointer hover:text-natural-accent transition-colors select-none"
                ...
                <th className="px-6 py-4 text-[0.7rem] font-bold uppercase tracking-widest text-natural-text-light border-b border-natural-border text-right">Actions</th>
              </tr>
            </thead>
            <tbody>
              {sortedUsers.map((user) => (
                <tr key={user.id} className={cn(
                  ...
                  user.status === 'Archived' ? "bg-natural-bg/40 opacity-60 grayscale-[0.5]" : "hover:bg-natural-bg/20"
                )}>
                  <td className="px-6 py-4 border-b border-natural-border/50">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="text-sm font-bold text-natural-text-main tracking-tight">{user.name}</p>
                  ...
                  <td className="px-6 py-4 border-b border-natural-border/50">
                    <div className="space-y-1">
                      <p className="text-xs text-natural-text-main font-semibold flex items-center gap-1.5 antialiased">
                        <Phone className="w-3.5 h-3.5 text-natural-accent" /> {user.phone}
                      </p>
                  ...
                  <td className="px-6 py-4 border-b border-natural-border/50 text-right">
                    <div className="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity min-h-[32px]">
                      {user.id !== 0 && user.status !== 'Archived' ? (
                        <>
                          <button 
                        ...
                        <span className="text-[0.6rem] font-bold text-natural-text-light/40 uppercase tracking-widest bg-natural-bg/50 px-2 py-1 rounded">Archived - Read Only</span>
                      ) : (
                        <span className="text-[0.6rem] font-bold text-natural-text-light/40 uppercase tracking-widest bg-natural-bg/50 px-2 py-1 rounded">System Protected</span>
                      )}
                    </div>
                  </td>
                </tr>
          ...
          {sortedUsers.length === 0 && (
            <div className="py-20 text-center">
              <p className="text-natural-text-light font-serif italic">No users found matching your criteria</p>
            </div>
          )}
        </div>
      </div>
            ...
            <div className="p-6 space-y-5">
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  <label className="text-[0.65rem] font-bold text-natural-text-light uppercase tracking-widest pl-1">Full Name</label>
                  <input 
                    type="text" 
                    value={formData.name || ''}
                ...
                <div className="space-y-1.5">
                  <label className="text-[0.65rem] font-bold text-natural-text-light uppercase tracking-widest pl-1">Phone Number</label>
                  <input 
                    type="text" 
              ...
              <div className="space-y-1.5">
                <label className="text-[0.65rem] font-bold text-natural-text-light uppercase tracking-widest pl-1">Account Password</label>
                <div className="relative">
                  <input 
                    type="password" 
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  <label className="text-[0.65rem] font-bold text-natural-text-light uppercase tracking-widest pl-1">User Role</label>
                  <select 
                ...
                <div className="space-y-1.5">
                  <label className="text-[0.65rem] font-bold text-natural-text-light uppercase tracking-widest pl-1">Status</label>
                  <select 
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden animate-in zoom-in-95 duration-200 border border-natural-border">
            <div className="p-8 text-center">
              <div className={cn(
                ...
                {confirmAction.type === 'create' && <UserPlus className="w-8 h-8 text-white" />}
              ...
              <h3 className="text-xl font-serif font-bold text-natural-text-main mb-2">
                {confirmAction.type === 'create' ? 'Create New Account?' : 
              ...
              <p className="text-sm text-natural-text-light mb-8 leading-relaxed">
                {confirmAction.type === 'create' ? (
                  <>Are you sure you want to register <span className="font-bold text-natural-text-main">{confirmAction.userName}</span> as a new user in the system?</>
                ) : confirmAction.type === 'edit' ? (
                  <>You are about to update the profile details for <span className="font-bold text-natural-text-main">{confirmAction.userName}</span>. Continue?</>
                ) : (
                  <>This will move <span className="font-bold text-natural-text-main">{confirmAction.userName}</span> to the archives. They will no longer be able to access active features.</>
                )}
              </p>
