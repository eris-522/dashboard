...
export function InventoryPage() {
  const { items, isLoading, addItem, updateStock, archiveItem, updateItem } =
    useInventory();
  ...
  const [sortStatus, setSortStatus] = React.useState<"asc" | "desc" | null>(
    null,
  );
  ...
  const handleUpdateStock = async () => {
    ...
    if (updateQuantity !== 0) {
      await updateStock(selectedItemId, updateQuantity);
    }

    if (
      updateItem &&
      (editName !== currentItem.name ||
        editCategory !== currentItem.category ||
        editMinStock !== currentItem.minStock)
    ) {
    ...
    await logAuditAction({
      ...
      details:
        updateQuantity !== 0
          ? `Adjusted stock by ${updateQuantity > 0 ? "+" : ""}${updateQuantity} ${currentItem.unit}`
          : `Modified inventory item details`,
    });
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-natural-text-light text-[0.8rem] font-medium uppercase tracking-wider">
            Monitor and restock catering supplies
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="glass-card p-6 flex items-center gap-4 bg-white">
          ...
          <div>
            ...
            <h4 className="text-2xl font-bold font-serif text-natural-text-main text-[0.9rem] uppercase tracking-tighter">
              {stats.lastUpdate}
            </h4>
          </div>
        </div>
          ...
          {isLoading ? (
            ...
            <div className="flex flex-col items-center justify-center py-20 text-natural-text-light">
              ...
              <p className="text-[0.7rem] uppercase tracking-widest mt-1">
                Start by adding some catering supplies
              </p>
            </div>
          ) : (
                    ...
                    >
                      <td className="px-6 py-5 border-b border-natural-border/50">
                        <div>
                          ...
                          <p className="text-[0.65rem] text-natural-text-light font-bold uppercase tracking-widest mt-1">
                            {item.category}
                          </p>
                        </div>
                      </td>
            ...
            <div className="p-6 space-y-6">
              ...
              {!selectedItemId ? (
                <div className="space-y-2">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-[0.6rem] font-bold text-natural-accent bg-natural-accent/10 px-2 py-0.5 rounded">
                      Needs Attention
                    </span>
                  </div>
                    ...
                    {items
                      .filter(
                        (i) =>
                          ...
                          (i.status === "Critical" || i.status === "Low Stock"),
                      )
                      .sort((a, b) => {
                      ...
                      .map((item) => (
                        ...
                        >
                          ...
                          <div className="pl-2 flex justify-between items-center">
                            <div>
                              <p className="text-base font-bold text-natural-text-main">
                                {item.name}
                              </p>
                ...
                <div className="p-4 bg-natural-bg/50 rounded-2xl border border-natural-border space-y-4">
                  ...
                  <div className="pt-3 border-t border-natural-border/50">
                    <p className="text-[10px] text-natural-text-light">
                      Current Stock:{" "}
                      {items.find((i) => i.id === selectedItemId)?.stock}/
                      {items.find((i) => i.id === selectedItemId)?.minStock}{" "}
                      {items.find((i) => i.id === selectedItemId)?.unit}
                    </p>
                    <button
                      onClick={() => setSelectedItemId(null)}
                      className="text-[10px] text-natural-accent font-bold uppercase mt-1 hover:underline"
                    >
                      Change Item
                    </button>
                  </div>
              ...
              <div className="space-y-4">
                <div className="space-y-1">
                  ...
                  <p className="text-[10px] text-natural-text-light text-center mt-2 italic">
                    Use negative values for reduction/outgoing
                  </p>
                </div>
              </div>
            </div>
              ...
              <button
                onClick={() => {
                  ...
                  if (editName && !nameRegex.test(editName)) {
                    setFormError(
                      "Item name cannot contain numbers or special characters.",
                    );
                    return;
                  }
                  setFormError(null);
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden animate-in zoom-in-95 duration-200 border border-natural-border relative">
            <div className="p-8 text-center">
              ...
              <p className="text-sm text-natural-text-light mb-8 leading-relaxed">
                You are about to update the details and stock for{" "}
                <span className="font-bold text-natural-text-main">
                  {items.find((i) => i.id === selectedItemId)?.name || editName}
                </span>
                . Continue?
              </p>
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden animate-in zoom-in-95 duration-200 border border-natural-border relative">
            <div className="p-8 text-center">
              ...
              <p className="text-sm text-natural-text-light mb-8 leading-relaxed">
                ...
                <span className="font-bold text-natural-text-main">
                  {items.find((i) => i.id === itemToArchive)?.name}
                </span>{" "}
                to the archives? It will be hidden from active views.
              </p>
                ...
                <button
                  onClick={async () => {
                    if (itemToArchive !== null) {
                      const item = items.find((i) => i.id === itemToArchive);
