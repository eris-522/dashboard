          ...
          <main className="flex-1 ml-56 p-8">
              ...
              <div>
                <h2 className="text-2xl font-serif font-bold text-natural-text-main">
                  {activeTab === 'dashboard' ? 'Executive Dashboard' : 
                   ...
                   activeTab === 'booking' ? 'Event Logistics' : 
                   activeTab === 'menu' ? 'Culinary Workspace' : 
                   activeTab === 'packages' ? 'Package Bundles' : 
                   activeTab === 'inventory' ? 'Inventory Control' : 
                   activeTab === 'analytics' ? 'Insights & Performance' : 
                   activeTab === 'audit-trail' ? 'Administrative Log' : 
                   activeTab === 'setting' ? 'Account Preferences' : 'Catering Workspace'}
                </h2>
                <p className="text-natural-text-light text-[0.8rem] font-medium uppercase tracking-wider">
                  {activeTab === 'dashboard' ? 'Business Intelligence & Logistics' : 
                   activeTab === 'user' ? 'User Roles & Access Control' : 
                   activeTab === 'booking' ? 'Scheduled Catering & Operations' : 
                   activeTab === 'menu' ? 'Menu Development & Inventory Link' : 
                   activeTab === 'packages' ? 'Tiered Event Solutions' : 
                   activeTab === 'inventory' ? 'Asset Management & Fulfillment' : 
                   activeTab === 'analytics' ? 'Data-Driven Growth Strategies' : 
                   activeTab === 'audit-trail' ? 'Chronological System Activity Trail' : 
                   activeTab === 'setting' ? 'Workflow & Environment Configuration' : 'Management Console'}
                </p>
              </div>
              ...
              <div className="flex items-center gap-6">
                ...
                <div className="flex items-center gap-2 relative">
                  <button 
                    ...
                    className={cn(
                      ...
                      isNotificationOpen && "ring-2 ring-natural-accent/20 bg-natural-bg"
                    )}
                  >
                ...
                <div className="flex items-center gap-3">
                  <div className="text-right">
                    <p className="text-xs font-bold text-natural-text-main uppercase tracking-tight">{currentUser.name}</p>
                    <p className="text-[9px] font-bold text-natural-text-light uppercase tracking-widest opacity-70">{currentUser.role} Account</p>
                  </div>
            ...
            <div className="max-w-7xl">
              {activeTab === 'dashboard' ? <Dashboard onNavigate={setActiveTab} /> : 
               ...
               activeTab === 'packages' ? <PackagePage /> :
               activeTab === 'inventory' ? <InventoryPage /> :
               activeTab === 'analytics' ? <AnalyticsPage /> :
               activeTab === 'audit-trail' ? <AuditTrailPage /> :
               activeTab === 'setting' ? <SettingsPage /> :
               <div className="p-20 text-center glass-card">
                 <p className="text-natural-text-light font-serif italic">This section ({activeTab}) is under construction.</p>
               </div>
              }
            </div>
